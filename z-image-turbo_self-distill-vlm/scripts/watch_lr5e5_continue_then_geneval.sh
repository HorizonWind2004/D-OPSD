...
EXP_ROOT="${EXP_ROOT:-/mnt/hdfs/jixie/checkpoints/zimage_reconstruction_train/rgb_stage1_zimage_reca_style_lora_lr5e5_2k_continue_from1k}"
OUT_ROOT="${OUT_ROOT:-/opt/tiger/why-reca/outputs/geneval_zimage_recon_lr5e5_continue_2k_25step_512}"
...
# Wait for 2k first so Geneval does not compete with active continuation training.
step2000_adapter="$(wait_for_adapter \
  "step2000/final adapter" \
  "$EXP_ROOT/checkpoints/step_002000/recon" \
  "$EXP_ROOT/checkpoints/final/recon")"

step1500_adapter="$(wait_for_adapter \
  "step1500 adapter" \
  "$EXP_ROOT/checkpoints/step_001500/recon")"

run_geneval_for_adapter "finetuned_lr5e5_step1500" "$step1500_adapter"
run_geneval_for_adapter "finetuned_lr5e5_step2000" "$step2000_adapter"

echo "[$(date)] completed Geneval for step1500 and step2000"
